...
bool sysfs::exists(const std::string &path, const std::string &file) const
{
	std::ostringstream log;
	if (logger::isDebug()) {
		log << "pstate-frequency [psfreq_sysfs.cpp]: sysfs::exists" << std::endl;
		logger::d(log);
	}
	...
	if (!inputFile.is_open()) {
		if (logger::isDebug()) {
			log << "\tFile does not exist" << std::endl;
			logger::d(log);
		}
		return false;
	}
	if (logger::isDebug()) {
		log << "\tFile exists, closing" << std::endl;
		logger::d(log);
	}
	inputFile.close();
	return true;
...
bool sysfs::exists(const std::string &file) const
{
