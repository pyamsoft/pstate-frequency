...
ifeq ($(INCLUDE_UDEV_RULE), 1)
	...
	@install -m 644 assets/udev/$(UDEV) \
		$(DESTDIR)/$(ETCDIR)/udev/rules.d/$(UDEV)
...
ifeq ($(INCLUDE_BASH_COMPLETION), 1)
	@echo -n "  UNINSTALL  "
	@echo "$(DESTDIR)/$(ETCDIR)/bash_completion.d/$(EXEC_NAME)"
