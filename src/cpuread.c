...
static char *internal_safe_read_cpu(struct cpu_t *const cpu,
...
{
	return internal_safe_read_cpu(cpu, file_name);
}

int32_t get_online_status(struct cpu_t *const cpu, char *const file_name)
{
	return str_to_num(internal_safe_read_cpu(cpu, file_name));
}
#endif

char *get_driver(struct cpu_t *const cpu)
{
	return internal_safe_read_cpu(cpu, FILE_SCALING_DRIVER);
}

char *get_scheduler(struct cpu_t *const cpu)
{
	return internal_safe_read_cpu(cpu, FILE_IO_SCHEDULER);
}

char *get_governor(struct cpu_t *const cpu)
{
	return internal_safe_read_cpu(cpu, FILE_SCALING_GOVERNOR);
}

/*
 * Helper function to safely read cpu values 
 */
static char *internal_safe_read_cpu(struct cpu_t *const cpu, char *const file_name)
{
        log_debug("Open file %s\n", file_name);
        FILE *file = fopen(file_name, "r");
        char *result = internal_get(cpu, file, file_name);
        log_debug("result is: %s", result);
        fclose(file);
        log_debug("file %s closed\n", file_name);
        return result;
}
...
{
        return str_to_num(internal_safe_read_cpu(cpu, FILE_SCALING_MAX_FREQ));
}
...
{
        return str_to_num(internal_safe_read_cpu(cpu, FILE_SCALING_MIN_FREQ));
}
...
{
        return str_to_num(internal_safe_read_cpu(cpu, FILE_INFO_MAX_FREQ));
}
...
{
        return str_to_num(internal_safe_read_cpu(cpu, FILE_INFO_MIN_FREQ));
}
...
{
        if (has_pstate(cpu)) {
                return str_to_num(internal_safe_read_cpu(cpu, FILE_PSTATE_TURBO));
        }
